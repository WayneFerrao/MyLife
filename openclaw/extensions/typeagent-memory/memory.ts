...
async function loadModule(): Promise<ConversationMemoryModule> {
  try {
    ...
    // CJS packages imported via ESM dynamic import put exports under .default
    return ((mod.default ?? mod) as ConversationMemoryModule);
  } catch (err) {
    throw new Error(
      "TypeAgent conversation-memory not available. " +
        "If TypeAgent is still building, wait for the typeagent-build container to finish " +
        "then run: docker compose restart openclaw-gateway",
      { cause: err },
    );
  }
}
